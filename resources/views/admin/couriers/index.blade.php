            ...
            <div class="card">
                <div class="card-body">
                    <div class="d-flex align-items-center mb-3">
                        <h6 class="card-title my-3">Kurir</h6>
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center">
                                                ...
                                                <button type="button" class="mr-2 btn btn-sm btn-danger btn-icon-text"
                                                    data-bs-toggle="modal" data-bs-target="#deleteModal{{ $courier->id }}">
...
@endpush
@foreach ($couriers as $courier)
    <div class="modal fade" id="deleteModal{{ $courier->id }}" tabindex="-1" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel">Hapus "{{ $courier->name }}" ?</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="btn-close"></button>
                </div>
                <div class="modal-body">
                    Yakin ingin menghapus Kurir "{{ $courier->name }}" ?
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Tidak</button>
                    <form id="delete-courier-{{ $courier->id }}"
                        action="{{ route('admin.couriers.destroy', $courier) }}" method="post">
                        @csrf
                        @method('delete')
